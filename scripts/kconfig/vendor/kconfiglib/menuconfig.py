...
# Linux mconf-compatible instructions shown above the menu box
_MAIN_HELP_LINES = """
Arrow keys navigate the menu.  <Enter> selects submenus --->.
<Y> includes, <N> excludes, <M> modularizes features.  <Space> toggles.
Press <Esc><Esc> to exit, <?> for Help, </> for Search.
Legend: [*] built-in  [ ] excluded  <M> module  < > module capable
"""[1:-1].split("\n")

_MAIN_BUTTONS = "<Select>  <Exit>  <Help>  <Save>  <Load>"
...
_STYLES = {
    ...
    # Linux mconf/lxdialog layout: blue desktop and a light dialog body
    "linux": """
    path=fg:black,bg:white
    separator=fg:black,bg:white,bold
    list=fg:black,bg:white
    selection=fg:white,bg:blue,bold
    inv-list=fg:red,bg:white
    inv-selection=fg:red,bg:blue
...
def _init():
    ...
    # Centered lxdialog-compatible outer frame and inner menu border
    _frame_win = _styled_win("frame")
    _menu_box_win = _styled_win("frame")

    # Top row inside the dialog, with menu path
...
def _resize_main():
    # Resize and center the Linux mconf-style dialog.

    global _menu_scroll

    screen_height, screen_width = _stdscr.getmaxyx()

    frame_height = min(max(screen_height - 2, 1), 30)
    frame_width = min(max(screen_width - 4, 1), 100)
    frame_y = max((screen_height - frame_height)//2, 0)
    frame_x = max((screen_width - frame_width)//2, 0)

    help_win_height = _SHOW_HELP_HEIGHT if _show_help else len(_MAIN_HELP_LINES)
    menu_win_height = frame_height - help_win_height - 7

    if menu_win_height >= 1 and frame_width >= 20:
        _place_window(_frame_win, frame_height, frame_width, frame_y, frame_x)
        _place_window(_top_sep_win, 1, frame_width - 2,
                      frame_y, frame_x + 1)
        _place_window(_path_win, 1, frame_width - 4,
                      frame_y + 1, frame_x + 2)
        _place_window(_help_win, help_win_height, frame_width - 4,
                      frame_y + 2, frame_x + 2)

        menu_box_y = frame_y + 2 + help_win_height
        _place_window(_menu_box_win, menu_win_height + 2, frame_width - 4,
                      menu_box_y, frame_x + 2)
        _place_window(_menu_win, menu_win_height, frame_width - 6,
                      menu_box_y + 1, frame_x + 3)
        _place_window(_bot_sep_win, 1, frame_width - 4,
                      frame_y + frame_height - 2, frame_x + 2)
    else:
        ...
        for win in (_frame_win, _menu_box_win, _path_win, _top_sep_win,
                    _menu_win, _bot_sep_win, _help_win):
            _place_window(win, 1, max(screen_width, 1), 0, 0)
...
def _place_window(win, height, width, row, column):
    """Resize and move a curses window safely across terminal resizes."""
    win.mvwin(0, 0)
    win.resize(max(height, 1), max(width, 1))
    win.mvwin(max(row, 0), max(column, 0))
...
def _centered_menu_origin(lines, height, width, center_vertically):
    """Return the row and column for a centered block of menu entries."""
    if not lines:
        return 0, 0

    row = max((height - len(lines))//2, 0) if center_vertically else 0
    block_width = min(max(len(line) for line in lines), max(width - 2, 0))
    column = max((width - block_width)//2, 0)
    return row, column
...
def _draw_main():
    ...
    term_width = _width(_top_sep_win)

    # Apply the background through ncurses itself. bkgd(), unlike bkgdset(),
    # updates every existing root-window cell and remains portable across
    # terminal emulators.
    _stdscr.bkgd(" ", _style["body"])
    _stdscr.erase()
    _stdscr.touchwin()
    _stdscr.noutrefresh()
    ...
    _frame_win.erase()
    _frame_win.border()
    _frame_win.noutrefresh()

    _menu_box_win.erase()
    _menu_box_win.border()
    _menu_box_win.noutrefresh()
    ...
    # Draw the visible entries as one centered block, matching the placement
    # used by the Linux menuconfig dialog. Long menus still occupy the full
    # window so scrolling never causes the list to jump vertically.
    visible_end = min(_menu_scroll + _height(_menu_win), len(_shown))
    rendered = [(i, _shown[i], _node_str(_shown[i]))
                for i in range(_menu_scroll, visible_end)]
    row_offset, column_offset = _centered_menu_origin(
        [line for _, _, line in rendered],
        _height(_menu_win),
        _width(_menu_win),
        len(_shown) <= _height(_menu_win))

    for screen_row, (i, node, line) in enumerate(rendered, row_offset):
        ...
        _safe_addstr(_menu_win, screen_row, column_offset, line, style)
    ...
    _safe_addstr(_bot_sep_win, 0,
                 max((_width(_bot_sep_win) - len(_MAIN_BUTTONS))//2, 0),
                 _MAIN_BUTTONS)
