...
/* Minimal Linux init that starts the XFS runtime test on either architecture. */

typedef unsigned long edge_word_t;

static char shell_path[] = "/bin/sh";
static char script_path[] = "/root/xfs_runtime_test.sh";
static char failure_message[] = "XFS_RUNTIME_INIT_EXEC_FAILED\n";
...
static long edge_linux_syscall3(edge_word_t number, edge_word_t first,
    ...
    return (long)rax;
#else
#error "The XFS runtime init supports only AArch64 and x86_64"
#endif
}
