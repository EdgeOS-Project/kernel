...
/* Host-side XFS reader test against a real mkfs.xfs image. */
...
static void load_path(edge_xfs_reader_t *reader, const char *path,
                      edge_xfs_inode_t *inode) {
    edge_xfs_inode_t current;
    char component[256];
    const char *cursor = path;

    assert(edge_xfs_inode_load(reader, reader->root_inode, &current) == 0);
    while (*cursor == '/') ++cursor;
    while (*cursor) {
        uint32_t length = 0;
        ...
        component[length] = 0;
        assert(edge_xfs_directory_lookup(
            reader, &current, component, &inode_number) == 0);
        if (edge_xfs_inode_load(reader, inode_number, &current) != 0) {
            fprintf(stderr, "failed to load %s inode %llu\n", component,
...
int main(int argc, char **argv) {
    ...
    assert(edge_xfs_reader_init(&reader, &device) == 0);
    assert(reader.block_size == 4096u);
    assert(edge_xfs_inode_load(&reader, reader.root_inode, &inode) == 0);
    ...
    assert(edge_xfs_inode_read(
        &reader, &inode, 0, buffer, sizeof(buffer)) == 19);
    assert(strcmp(buffer, "edgeos-xfs-runtime\n") == 0);
    ...
    load_path(&reader, "/btree.bin", &inode);
    assert(inode.format == 3u);
    memset(buffer, 0, sizeof(buffer));
    assert(edge_xfs_inode_read(
        &reader, &inode, 40u * 8192u, buffer, sizeof(buffer)) ==
        sizeof(buffer));
    for (uint32_t index = 0; index < sizeof(buffer); ++index)
        assert((unsigned char)buffer[index] == 0x5au);
    memset(buffer, 0xff, sizeof(buffer));
    assert(edge_xfs_inode_read(
        &reader, &inode, 40u * 8192u + 4096u, buffer, sizeof(buffer)) ==
        sizeof(buffer));
    for (uint32_t index = 0; index < sizeof(buffer); ++index)
        assert(buffer[index] == 0);

    fclose(image);
    puts("xfs_reader_unit: PASS");
    return 0;
}
