...
/* Freestanding Linux BPF instruction-array syscall ABI probe. */
...
#define SYS_bpf 280
#else
#error "bpf_insn_array_abi_probe requires a Linux 64-bit architecture"
#endif
...
static long create_map(uint32_t key_size, uint32_t value_size,
    ...
    attribute.map_type = BPF_MAP_TYPE_INSN_ARRAY;
...
static int test_instruction_array(void) {
    struct instruction_array_value value = {0u};
    uint32_t key = 0u;
    ...
    failures += expect_result(
        "bad-key-size", create_map(8u, sizeof(value), 2u, 0u, 0u),
        -EINVAL);
    failures += expect_result(
        "bad-value-size", create_map(4u, 8u, 2u, 0u, 0u), -EINVAL);
    failures += expect_result(
        "bad-map-flags", create_map(
            4u, sizeof(value), 2u, BPF_F_RDONLY, 0u), -EINVAL);
    failures += expect_result(
        "bad-map-extra", create_map(
            4u, sizeof(value), 2u, 0u, 1u), -EINVAL);
    descriptor = create_map(4u, sizeof(value), 2u, 0u, 0u);
    if (descriptor < 0)
        return failures + expect_result("create", descriptor, 0);

    memset(&value, 0xff, sizeof(value));
    failures += expect_result(
        "initial-lookup", map_element(
            BPF_MAP_LOOKUP_ELEM, descriptor, &key, &value, 0u), 0);
    failures += expect_result("initial-original", value.original_offset, 0);
    failures += expect_result("initial-translated", value.translated_offset, 0);
    failures += expect_result("initial-jitted", value.jitted_offset, 0);
    failures += expect_result("initial-reserved", value.reserved, 0);
    value.original_offset = 7u;
    value.reserved = UINT32_MAX;
    failures += expect_result(
        "update", map_element(BPF_MAP_UPDATE_ELEM, descriptor,
                               &key, &value, BPF_ANY), 0);
    memset(&value, 0xff, sizeof(value));
    failures += expect_result(
        "lookup", map_element(BPF_MAP_LOOKUP_ELEM, descriptor,
                               &key, &value, 0u), 0);
    failures += expect_result("lookup-original", value.original_offset, 7);
    failures += expect_result("lookup-translated", value.translated_offset, 0);
    failures += expect_result("lookup-jitted", value.jitted_offset, 0);
    failures += expect_result("lookup-reserved", value.reserved, 0);
    failures += expect_result(
        "noexist", map_element(BPF_MAP_UPDATE_ELEM, descriptor,
                                &key, &value, BPF_NOEXIST), -EEXIST);
    value.original_offset = 8u;
    failures += expect_result(
        "exist", map_element(BPF_MAP_UPDATE_ELEM, descriptor,
                              &key, &value, BPF_EXIST), 0);
    value.translated_offset = 1u;
    failures += expect_result(
        "translated", map_element(BPF_MAP_UPDATE_ELEM, descriptor,
                                    &key, &value, BPF_ANY), -EINVAL);
    value.translated_offset = 0u;
    value.jitted_offset = 1u;
    failures += expect_result(
        "jitted", map_element(BPF_MAP_UPDATE_ELEM, descriptor,
                               &key, &value, BPF_ANY), -EINVAL);
    key = 2u;
    failures += expect_result(
        "update-out", map_element(BPF_MAP_UPDATE_ELEM, descriptor,
                                   &key, &value, BPF_ANY), -E2BIG);
    failures += expect_result(
        "lookup-out", map_element(BPF_MAP_LOOKUP_ELEM, descriptor,
                                   &key, &value, 0u), -ENOENT);
    key = 0u;
    failures += expect_result(
    ...
    failures += expect_result(
        "frozen-delete", map_element(BPF_MAP_DELETE_ELEM, descriptor,
                                      &key, 0, 0u), -EPERM);
...
START_ATTRIBUTES void _start(void) {
    int failures = test_instruction_array();

    if (failures) {
        print_text("bpf-insn-array: FAIL\n");
        raw_syscall6(SYS_exit, 1, 0, 0, 0, 0, 0);
    }
    print_text("bpf-insn-array: PASS\n");
