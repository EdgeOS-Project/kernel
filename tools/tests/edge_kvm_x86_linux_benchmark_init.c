/* Minimal x86_64 Linux PID 1 for nested-KVM boot and performance acceptance. */
...
void _start(void)
{
    ...
    print_text("EDGE_X86_LINUX_BOOT_PASS\n");
    if (linux_syscall6(LINUX_SYS_UNAME, (long)&identity, 0, 0, 0, 0, 0) == 0) {
        print_text("EDGE_X86_LINUX_RELEASE=");
        print_text(identity.release);
        print_text("\nEDGE_X86_LINUX_MACHINE=");
    ...
    print_metric("EDGE_X86_LINUX_CPU_ITERATIONS=", CPU_ITERATIONS);
    print_metric("EDGE_X86_LINUX_CPU_NS=", end - start);
    print_metric("EDGE_X86_LINUX_CPU_CHECKSUM=", state);
    ...
    if ((long)(uintptr_t)memory < 0) {
        print_text("EDGE_X86_LINUX_MEMORY_MAP_FAIL\n");
        power_off();
    }
    start = monotonic_nanoseconds();
    ...
    print_metric("EDGE_X86_LINUX_MEMORY_BYTES=", bytes);
    print_metric("EDGE_X86_LINUX_MEMORY_NS=", end - start);
    print_metric("EDGE_X86_LINUX_MEMORY_MIB_PER_SEC=", memory_rate);
    print_metric("EDGE_X86_LINUX_MEMORY_CHECKSUM=", checksum);

    start = monotonic_nanoseconds();
    for (index = 0; index < GETPID_ITERATIONS; index++)
        checksum ^= (uint64_t)linux_syscall6(
            LINUX_SYS_GETPID, 0, 0, 0, 0, 0, 0);
    end = monotonic_nanoseconds();
    print_metric("EDGE_X86_LINUX_GETPID_CALLS=", GETPID_ITERATIONS);
    print_metric("EDGE_X86_LINUX_GETPID_NS=", end - start);
    print_metric("EDGE_X86_LINUX_FINAL_CHECKSUM=", checksum);
    print_text("EDGE_X86_LINUX_BENCHMARK_PASS\n");
    power_off();
}
