...
int kernel_io_file_range_sync(int32_t descriptor, int data_only) {
    assert(descriptor == 70 || descriptor == 71 || descriptor == 72);
    assert(data_only == 0 || data_only == 1);
    ++g_file_sync_data_calls;
    return 0;
}
...
int64_t kernel_fd_operation_file_range(
    ...
    if (request->operation == KERNEL_IO_FILE_RANGE_QUERY) {
        ...
        request->information->kind = descriptor == 75 ?
            KERNEL_IO_FILE_REGULAR : KERNEL_IO_FILE_PIPE;
        request->information->readable = descriptor != 74 &&
            descriptor != 75;
        request->information->writable = descriptor == 74 ||
            descriptor == 75;
        request->information->size = descriptor == 75 ? 64u : 0u;
        return 0;
    }
    if (descriptor == 73 &&
    ...
    if (descriptor == 75 &&
        request->operation == KERNEL_IO_FILE_RANGE_COMPLETE_WRITE) {
        ++g_file_write_completions;
        return 0;
    }
    if (descriptor == 75 &&
        request->operation == KERNEL_IO_FILE_RANGE_SYNC_DATA) {
        ++g_file_sync_data_calls;
        return 0;
    }
...
int main(void) {
    ...
    }
    {
        ...
        memset(mapped_sqe, 0, 128u);
        mapped_sqe->opcode = 64u;
        mapped_sqe->user_data = 0x535145313238u;
        for (uint32_t index = 0; index < sizeof(command_payload); ++index)
            ((uint8_t *)(void *)mapped_sqe)[48u + index] =
                (uint8_t)(index ^ 0xa5u);
        ...
        *page_u32(&extended_sq_ring, 4u) = 1u;
        {
            ...
            assert(kernel_io_uring_take_submission(
                       ...
                       command_payload, &entries_consumed,
                       &layout_result) == 0);
            assert(entries_consumed == 1u);
            assert(layout_result == 0);
        }
        assert(extended_submission.opcode == 64u);
        assert(extended_submission.user_data == 0x535145313238u);
        for (uint32_t index = 0; index < sizeof(command_payload); ++index)
            assert(command_payload[index] == (uint8_t)(index ^ 0xa5u));
    ...
    }
    {
        ...
        *page_u32(&restricted_sq_ring,
        ...
        assert(kernel_io_uring_take_submission(
                   ...
                   0, &entries_consumed, &layout_result) == 0);
        assert(entries_consumed == 1u);
    ...
    }
    {
        ...
        *page_u32(&mixed_sq_ring, 4u) = 2u;
        ...
        for (uint32_t index = 0; index < sizeof(command_payload); ++index)
            assert(command_payload[index] == (uint8_t)(0x5au + index));
    ...
    }
    {
        ...
        *page_u32(&no_array_sq_ring, no_array_parameters.sq_off.tail) = 1u;
        ...
        assert(kernel_io_uring_take_submission(
                   ...
                   0, &entries_consumed, &layout_result) ==
               -EDGE_LINUX_EAGAIN);
        test_page_release(0, &no_array_sq_ring);
        test_page_release(0, &no_array_sqes);
        kernel_io_uring_release(second_ring_id);
    ...
    *page_u32(&cq_ring, parameters.cq_off.head) = 1u;
    ...
    }
    {
        ...
        independent_submission.descriptor = 75;
        independent_submission.operation_flags = 0x12u;
        independent_submission.user_data = 0x494e4457464c4147ull;
        memset(g_file_data, 0, sizeof(g_file_data));
        g_file_sync_data_calls = 0u;
        g_file_write_completions = 0u;
        g_ready_descriptor = 75;
        g_ready_generation = g_descriptor_generation[75];
        assert(kernel_io_uring_worker_add(
                   second_ring_id, 42, &independent_submission,
        ...
        assert(independent_completion[2].user_data ==
                   independent_submission.user_data &&
               independent_completion[2].result ==
                   (int32_t)sizeof(output));
        assert(memcmp(g_file_data + 64u, output, sizeof(output)) == 0);
        assert(g_file_write_completions == 1u);
        ...
        fixed_vectors[0].iov_base = output_address;
        fixed_vectors[0].iov_len = 5u;
        fixed_vectors[1].iov_base = second_page.iov_base;
        fixed_vectors[1].iov_len = sizeof(output) - 5u;
        memcpy(&g_pages[first_page][0x80u], output, 5u);
        memcpy(&g_pages[first_page + 1u][0x100u],
               output + 5u, sizeof(output) - 5u);
        independent_submission.opcode = 2u;
        independent_submission.descriptor = 74;
        independent_submission.address = 0xbeef0000u;
        independent_submission.length = 2u;
        independent_submission.user_data = 0x4f52445645435452ull;
        memset(g_file_data, 0, sizeof(g_file_data));
        g_ready_descriptor = 74;
        g_ready_generation = g_descriptor_generation[74];
        assert(kernel_io_uring_worker_add(
        ...
        assert(g_references[first_page] == 1u);
        assert(g_references[first_page + 1u] == 1u);
        assert(kernel_io_uring_completion_count(second_ring_id) == 7u);
        assert(independent_completion[6].user_data ==
                   independent_submission.user_data &&
               independent_completion[6].result ==
                   (int32_t)sizeof(output));
        assert(memcmp(g_file_data, output, sizeof(output)) == 0);
        fixed_vectors[0].iov_base = output_address;
        fixed_vectors[0].iov_len = 5u;
        fixed_vectors[1].iov_base = second_page.iov_base;
        fixed_vectors[1].iov_len = sizeof(output) - 5u;
        independent_submission.opcode = 1u;
        independent_submission.descriptor = 73;
        independent_submission.user_data = 0x4f52445645434341ull;
        g_ready_descriptor = -1;
        assert(kernel_io_uring_worker_add(
                   second_ring_id, 42, &independent_submission,
                   KERNEL_IO_READ_CURRENT, 0u,
                   fixed_vectors, 2u, 0xabc000u) == 0);
        assert(g_references[first_page] == 2u);
        assert(g_references[first_page + 1u] == 2u);
        assert(kernel_io_uring_pending_cancel(
