...
/* Freestanding Linux BPF cgroup local-storage syscall ABI probe. */
...
#define SYS_bpf 280
#else
#error "bpf_cgrp_storage_abi_probe requires a Linux 64-bit architecture"
#endif
...
static long create_map(int btf_fd, uint32_t flags,
    ...
    attribute.map_type = BPF_MAP_TYPE_CGRP_STORAGE;
...
static int test_cgroup_storage(void) {
    ...
    if (btf_fd < 0)
        return expect_result("cgrp-btf-load", btf_fd, 0) + 1;
    failures += expect_result(
        "cgrp-no-prealloc-required", create_map(
            (int)btf_fd, 0u, 0u), -EINVAL);
    failures += expect_result(
        "cgrp-zero-max-required", create_map(
            (int)btf_fd, BPF_F_NO_PREALLOC, 1u), -EINVAL);
    map_fd = create_map((int)btf_fd, BPF_F_NO_PREALLOC, 0u);
    if (map_fd < 0) {
        failures += expect_result("cgrp-create", map_fd, 0);
        goto close_btf;
    }
    ...
    (void)raw_syscall6(
        ...
        (long)"/sys/fs/cgroup/edge-cgrp-storage", 0755, 0, 0, 0);
    cgroup_fd = (int)raw_syscall6(
        SYS_openat, AT_FDCWD,
        (long)"/sys/fs/cgroup/edge-cgrp-storage",
        O_RDONLY | O_DIRECTORY, 0, 0, 0);
    if (cgroup_fd < 0) {
        failures += expect_result("cgrp-open", cgroup_fd, 0);
        goto close_map;
    }

    failures += expect_result(
        "cgrp-bad-fd", map_element(
            BPF_MAP_UPDATE_ELEM, (int)map_fd, &bad_fd,
            &value, BPF_ANY), -EBADF);
    failures += expect_result(
        "cgrp-lookup-empty", map_element(
            BPF_MAP_LOOKUP_ELEM, (int)map_fd, &cgroup_fd,
            &output, 0), -ENOENT);
    failures += expect_result(
        "cgrp-exist-empty", map_element(
            BPF_MAP_UPDATE_ELEM, (int)map_fd, &cgroup_fd,
            &value, BPF_EXIST), -ENOENT);
    failures += expect_result(
        "cgrp-insert", map_element(
            BPF_MAP_UPDATE_ELEM, (int)map_fd, &cgroup_fd,
            &value, BPF_ANY), 0);
    failures += expect_result(
        "cgrp-lookup", map_element(
            BPF_MAP_LOOKUP_ELEM, (int)map_fd, &cgroup_fd,
            &output, 0), 0);
    failures += expect_result(
        "cgrp-value", (long)output, (long)value);
    failures += expect_result(
        "cgrp-noexist", map_element(
            BPF_MAP_UPDATE_ELEM, (int)map_fd, &cgroup_fd,
            &replacement, BPF_NOEXIST), -EEXIST);
    failures += expect_result(
        "cgrp-replace", map_element(
            BPF_MAP_UPDATE_ELEM, (int)map_fd, &cgroup_fd,
...
START_ATTRIBUTES void _start(void) {
    int failures = test_cgroup_storage();

    print_text(failures ? "BPF_CGRP_STORAGE_ABI_FAIL\n" :
                          "BPF_CGRP_STORAGE_ABI_PASS\n");
