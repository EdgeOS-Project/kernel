...
static int64_t sleep_until(uint64_t deadline, uint64_t remaining_user,
                           int write_remaining, void *registers) {
    g_deadline = deadline;
...
int main(void) {
    ...
    kernel_restart_block_prepare_nanosleep(&block, 100u, 77u, 1);
    ...
    assert(g_deadline == 100u && g_remaining_user == 77u &&
           g_write_remaining == 1 && g_registers == registers);
    ...
    kernel_restart_block_prepare_nanosleep(&block, 100u, 0u, 0);
    assert(kernel_restart_block_execute(
