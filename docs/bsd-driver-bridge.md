...
unchanged. The `third_party_kernel/freebsd-src/` checkout is an import reference
only and is not required to build or distribute EdgeOS. Each driver family has a manifest under
`config/bsd_drivers/manifests/` that pins the upstream repository commit and
locks the source tree. The source check must pass before an imported driver is
compiled. Every locked file must declare an allowed license or have an explicit
manifest exception with human-readable evidence. Advertising-clause BSD source
is rejected.
