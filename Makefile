...
process-wait-policy-unit: tools/tests/process_wait_policy_unit.c \
		$(SRC)/kernel/process_wait.c include/kernel/process_runtime.h
	@mkdir -p $(OUT)/tests
	@$(HOST_CC) -std=c11 -Wall -Wextra -Werror -I$(INC) \
		tools/tests/process_wait_policy_unit.c \
		$(SRC)/kernel/process_wait.c \
		-o $(OUT)/tests/process_wait_policy_unit
	@$(OUT)/tests/process_wait_policy_unit
...
epoll-runtime-unit: tools/tests/epoll_runtime_unit.c \
		$(SRC)/kernel/epoll_runtime.c include/kernel/event_runtime.h
	@mkdir -p $(OUT)/tests
	@$(HOST_CC) -std=c11 -Wall -Wextra -Werror -fno-builtin \
		-DEDGEOS_HOST_TEST -iquote $(INC) \
		tools/tests/epoll_runtime_unit.c \
		-o $(OUT)/tests/epoll_runtime_unit
	@$(OUT)/tests/epoll_runtime_unit
...
restart-block-unit: tools/tests/restart_block_unit.c \
		$(SRC)/kernel/restart_block.c include/kernel/restart_block.h
	@mkdir -p $(OUT)/tests
	@$(HOST_CC) -std=c11 -Wall -Wextra -Werror -iquote $(INC) \
...
boottime-discipline-unit: tools/tests/boottime_discipline_unit.c \
	...
	@$(HOST_CC) -std=c11 -Wall -Wextra -Werror -fno-builtin \
		-DEDGEOS_HOST_TEST \
		-iquote $(INC) tools/tests/boottime_discipline_unit.c \
		$(SRC)/kernel/boottime.c -o $(OUT)/tests/boottime_discipline_unit
	@$(OUT)/tests/boottime_discipline_unit

time-discipline-unit: tools/tests/time_discipline_unit.c \
		$(SRC)/kernel/time_discipline.c include/kernel/time_discipline.h
	@mkdir -p $(OUT)/tests
	@$(HOST_CC) -std=c11 -Wall -Wextra -Werror -fno-builtin \
		-DEDGEOS_HOST_TEST \
		-iquote $(INC) tools/tests/time_discipline_unit.c \
		$(SRC)/kernel/time_discipline.c \
		-o $(OUT)/tests/time_discipline_unit
	@$(OUT)/tests/time_discipline_unit

timerfd-realtime-jump-unit: tools/tests/timerfd_realtime_jump_unit.c \
		$(SRC)/kernel/timerfd.c $(SRC)/kernel/boottime.c \
		include/kernel/timerfd.h include/sys/boottime.h
	@mkdir -p $(OUT)/tests
	@$(HOST_CC) -std=c11 -Wall -Wextra -Werror -fno-builtin \
		-DEDGEOS_HOST_TEST -iquote $(INC) \
		tools/tests/timerfd_realtime_jump_unit.c \
		$(SRC)/kernel/timerfd.c $(SRC)/kernel/boottime.c \
		-o $(OUT)/tests/timerfd_realtime_jump_unit
	@$(OUT)/tests/timerfd_realtime_jump_unit

linux-module-unit: tools/tests/linux_module_unit.c \
		$(SRC)/kernel/linux_module.c include/kernel/linux_module.h
	@mkdir -p $(OUT)/tests
	@$(HOST_CC) -std=c11 -Wall -Wextra -Werror -fno-builtin \
		-DCONFIG_MODULES=1 -DCONFIG_BSD_DRIVER_MODULES=1 \
		-iquote $(INC) tools/tests/linux_module_unit.c \
		$(SRC)/kernel/linux_module.c \
		-o $(OUT)/tests/linux_module_unit
	@$(OUT)/tests/linux_module_unit
...
.PHONY: bpf-runtime-unit
bpf-runtime-unit: tools/tests/bpf_runtime_unit.c \
		$(SRC)/kernel/bpf.c include/kernel/bpf_runtime.h
	@mkdir -p $(OUT)/tests
	@$(HOST_CC) -std=c11 -Wall -Wextra -Werror -fno-builtin \
		-iquote $(INC) tools/tests/bpf_runtime_unit.c \
		$(SRC)/kernel/bpf.c \
		-o $(OUT)/tests/bpf_runtime_unit
	@$(OUT)/tests/bpf_runtime_unit
