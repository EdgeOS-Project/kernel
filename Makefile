...
X86_INITRAMFS_LOGIN_OVERRIDE = $(OUT)/x86_64-initramfs-rootfs-override
ARM64_OUT = $(OUT)/arm64
...
ARM64_ESP = $(OUT)/edgeos-arm64-uefi.img
...
ARM64_ROOTFS = $(OUT)/rootfs-alpine-arm64.img
ARM64_ROOTFS_SIZE_MB ?= 256
ARM64_ESP_SIZE_MB ?= 576
...
ARM64_LOGIN_OVERRIDE = $(OUT)/arm64-login-rootfs-override
X86_BSD_MODULE_DIRECTORY := \
	$(call strip_quotes,$(strip $(CONFIG_BSD_DRIVER_MODULE_DIRECTORY)))
ARM64_BSD_MODULE_DIRECTORY := \
	$(call strip_quotes,$(strip $(ARM64_CONFIG_BSD_DRIVER_MODULE_DIRECTORY)))
X86_BSD_MODULE_CONFIG := \
	$(call strip_quotes,$(strip $(CONFIG_BSD_DRIVER_MODULE_CONFIG)))
ARM64_BSD_MODULE_CONFIG := \
	$(call strip_quotes,$(strip $(ARM64_CONFIG_BSD_DRIVER_MODULE_CONFIG)))
ifeq ($(X86_BSD_MODULE_DIRECTORY),)
X86_BSD_MODULE_DIRECTORY := /usr/lib/edgeos/modules
endif
ifeq ($(ARM64_BSD_MODULE_DIRECTORY),)
ARM64_BSD_MODULE_DIRECTORY := /usr/lib/edgeos/modules
endif
ifeq ($(X86_BSD_MODULE_CONFIG),)
X86_BSD_MODULE_CONFIG := /etc/modules.load
endif
ifeq ($(ARM64_BSD_MODULE_CONFIG),)
ARM64_BSD_MODULE_CONFIG := /etc/modules.load
endif
X86_BSD_MODULE_DIRECTORY_REL := \
	$(patsubst /%,%,$(X86_BSD_MODULE_DIRECTORY))
ARM64_BSD_MODULE_DIRECTORY_REL := \
	$(patsubst /%,%,$(ARM64_BSD_MODULE_DIRECTORY))
X86_BSD_MODULE_CONFIG_REL := \
	$(patsubst /%,%,$(X86_BSD_MODULE_CONFIG))
ARM64_BSD_MODULE_CONFIG_REL := \
	$(patsubst /%,%,$(ARM64_BSD_MODULE_CONFIG))
X86_BSD_MODULE_OVERLAY_DIR := \
	$(X86_INITRAMFS_LOGIN_OVERRIDE)/$(X86_BSD_MODULE_DIRECTORY_REL)
ARM64_BSD_MODULE_OVERLAY_DIR := \
	$(ARM64_LOGIN_OVERRIDE)/$(ARM64_BSD_MODULE_DIRECTORY_REL)
X86_BSD_MODULE_CONFIG_OVERLAY := \
	$(X86_INITRAMFS_LOGIN_OVERRIDE)/$(X86_BSD_MODULE_CONFIG_REL)
ARM64_BSD_MODULE_CONFIG_OVERLAY := \
	$(ARM64_LOGIN_OVERRIDE)/$(ARM64_BSD_MODULE_CONFIG_REL)
...
ARM64_LINUX_WORK = work/arm64-linux
ARM64_LINUX_OUT = $(OUT)/arm64-linux
ARM64_LINUX_APK_URL ?= https://dl-cdn.alpinelinux.org/alpine/v3.24/main/aarch64/linux-lts-6.18.38-r0.apk
ARM64_LINUX_APK = $(ARM64_LINUX_WORK)/linux-lts.apk
ARM64_LINUX_TREE = $(ARM64_LINUX_WORK)/linux-lts
ARM64_LINUX_KERNEL = $(ARM64_LINUX_TREE)/boot/vmlinuz-lts
ARM64_LINUX_INITRAMFS = $(ARM64_LINUX_OUT)/initramfs.cpio.gz
ARM64_LINUX_STARTUP = $(ARM64_LINUX_OUT)/startup.nsh
ARM64_LINUX_ESP = $(OUT)/edgeos-arm64-linux.img
ROOTFS = $(OUT)/rootfs.img
ROOTFS_TOOL = tools/rootfs/mkrootfs.py
DEBIAN_ROOTFS_TOOL = tools/rootfs/build_debian_rootfs.py
DEBIAN_SUITE ?= trixie
DEBIAN_ROOTFS_SIZE_MB ?= 8192
DEBIAN_ROOTFS_DIR ?= rootfs/debian/$(ROOTFS_ARCH)
DEBIAN_ROOTFS_IMAGE ?= $(DEBIAN_ROOTFS_DIR)/rootfs.img
DEBIAN_LIVE_ROOTFS_SIZE_MB ?= 2688
DEBIAN_LIVE_ROOTFS_DIR ?= $(OUT)/rootfs-debian-live-x86_64
DEBIAN_LIVE_ROOTFS_IMAGE ?= $(OUT)/rootfs-debian-live-x86_64.img
DEBIAN_LIVE_SQUASHFS ?= $(OUT)/filesystem-debian-live-x86_64.squashfs
DEBIAN_LIVE_INITRAMFS ?= $(OUT)/live-initramfs-x86_64.img
DEBIAN_LIVE_INITRAMFS_TOOL = tools/live/build_live_initramfs.py
DEBIAN_LIVE_INIT_LAUNCHER_SOURCE = tools/live/live_init_launcher_x86_64.c
DEBIAN_LIVE_INIT_LAUNCHER ?= $(OUT)/live-init-launcher-x86_64
DEBIAN_LIVE_BUSYBOX ?= rootfs/alpine/x86_64/bin/busybox
DEBIAN_LIVE_MUSL_LOADER ?= rootfs/alpine/x86_64/lib/ld-musl-x86_64.so.1
DEBIAN_LIVE_ISO_TOOL = tools/live/build_x86_64_debian_live_iso.py
DEBIAN_LIVE_GRUB_CONFIG = $(CONFIG)/grub-debian-live.cfg
DEBIAN_LIVE_ISO ?= $(OUT)/edgeos-x86_64-debian-live.iso
ROOTFS_OVERRIDE = tools/rootfs/rootfs_override
ROOTFS_PROFILE ?= edgeos
ROOTFS_ARCH ?= x86_64
ROOTFS_ALPINE_DIR ?= rootfs/alpine/$(ROOTFS_ARCH)
ifeq ($(ROOTFS_PROFILE),alpine)
ROOTFS := $(OUT)/rootfs-alpine.img
ROOTFS_POPULATE_DIR := $(ROOTFS_ALPINE_DIR)
ROOTFS_EXTRA_DEPS := FORCE
QEMU_MEM_DEFAULT := 2048M
ROOTFS_SIZE_MB ?= 2048
else
ROOTFS_POPULATE_DIR :=
ROOTFS_EXTRA_DEPS := FORCE
QEMU_MEM_DEFAULT := 512M
# The default EdgeOS image now carries musl userland, dropbear, CA roots,
# openssl, Python, and glibc compatibility files.  768 MiB and 1024 MiB leave
# no free ext4 blocks in common developer builds, and debugfs can fail late
# writes after the image has already become stale.  Regenerate the image and
# keep headroom so `make ROOTFS_PROFILE=edgeos` produces a complete rootfs.
ROOTFS_SIZE_MB ?= 2048
endif
...
X11_BUILD_SCRIPT = $(PORTS_DIR)/build_x11_musl.sh
...
XSERVER_DIR ?= $(USR)/third_party/xserver
LIBXCB_DIR ?= $(USR)/third_party/libxcb
X11_PREFIX ?= $(OUT)/x11-install
X11_SYSROOT ?= $(OUT)/x11-sysroot
X11_ROOTFS_OVERRIDE ?= $(OUT)/x11-rootfs-override
X11_COMPONENTS ?= xserver
TLS_CA_BUNDLE_SRC ?= /etc/ssl/certs/ca-certificates.crt
GLIBC_LOADER_SRC ?= $(firstword \
	$(wildcard /lib64/ld-linux-x86-64.so.2) \
	$(wildcard /lib/x86_64-linux-gnu/ld-linux-x86-64.so.2) \
	$(wildcard /usr/lib/x86_64-linux-gnu/ld-linux-x86-64.so.2))
GLIBC_LIBC_SRC ?= $(firstword \
	$(wildcard /lib/x86_64-linux-gnu/libc.so.6) \
	$(wildcard /usr/lib/x86_64-linux-gnu/libc.so.6))
...
INITBIN ?= $(firstword \
	$(wildcard $(ROOTFS_OVERRIDE)/bin/initd) \
	$(wildcard $(US_BUILD)/init))
...
$(ROOTFS): $(ROOTFS_TOOL) $(ROOTFS_EXTRA_DEPS)
	@mkdir -p $(OUT)
	@echo "[rootfs] creating $(ROOTFS_PROFILE) ext4 rootfs image"
	@if command -v python3 >/dev/null 2>&1; then \
		python3 $(ROOTFS_TOOL) --output $(ROOTFS) --size-mb $(ROOTFS_SIZE_MB) --fs ext4 $(if $(ROOTFS_POPULATE_DIR),--populate-dir $(ROOTFS_POPULATE_DIR),) --override-dir $(if $(filter $(ROOTFS_PROFILE),alpine),$(OUT)/.empty-rootfs-override,$(ROOTFS_OVERRIDE)); \
	else \
...
$(X86_INITRAMFS_LOGIN_OVERRIDE)/.stamp: \
		$(BSD_BRIDGE_LOADABLE_X86_MODULES) $(DOT_CONFIG)
	@case "$(X86_BSD_MODULE_DIRECTORY)" in /*) ;; *) \
		echo "[config] BSD_DRIVER_MODULE_DIRECTORY must be absolute"; \
		exit 1;; esac
	@case "$(X86_BSD_MODULE_CONFIG)" in /*) ;; *) \
		echo "[config] BSD_DRIVER_MODULE_CONFIG must be absolute"; \
		exit 1;; esac
	@mkdir -p "$(X86_INITRAMFS_LOGIN_OVERRIDE)/etc" \
		"$(X86_BSD_MODULE_OVERLAY_DIR)" \
		"$(dir $(X86_BSD_MODULE_CONFIG_OVERLAY))"
	@awk -F: 'BEGIN { OFS = FS } $$1 == "root" { $$2 = "" } { print }' rootfs/alpine/x86_64/etc/shadow > $(X86_INITRAMFS_LOGIN_OVERRIDE)/etc/shadow
	@cp rootfs/alpine/x86_64/etc/inittab $(X86_INITRAMFS_LOGIN_OVERRIDE)/etc/inittab
	@printf 'console\nttyS0\ntty1\ntty2\ntty3\ntty4\n' > $(X86_INITRAMFS_LOGIN_OVERRIDE)/etc/securetty
	@: > "$(X86_BSD_MODULE_CONFIG_OVERLAY)"
	@for module in $(BSD_BRIDGE_LOADABLE_X86_MODULES); do \
		name=$$(basename "$$module"); \
		cp "$$module" "$(X86_BSD_MODULE_OVERLAY_DIR)/$$name"; \
		printf '%s\n' "$$name" >> \
			"$(X86_BSD_MODULE_CONFIG_OVERLAY)"; \
	done
	@touch $@

$(X86_INITRAMFS): $(INITRAMFS_TOOL) $(INITRAMFS_BASE_MANIFEST) $(X86_INITRAMFS_LOGIN_OVERRIDE)/.stamp FORCE
	@mkdir -p $(dir $@)
	python3 $(INITRAMFS_TOOL) \
		--source rootfs/alpine/x86_64 \
		--overlay $(X86_INITRAMFS_LOGIN_OVERRIDE) \
		--manifest $(INITRAMFS_BASE_MANIFEST) \
		--output $@ \
		--owner 0:0 \
		--mtime 0 \
...
$(ARM64_LOGIN_OVERRIDE)/.stamp: \
		$(BSD_BRIDGE_LOADABLE_ARM64_MODULES) $(ARM64_DOT_CONFIG)
	@case "$(ARM64_BSD_MODULE_DIRECTORY)" in /*) ;; *) \
		echo "[config] BSD_DRIVER_MODULE_DIRECTORY must be absolute"; \
		exit 1;; esac
	@case "$(ARM64_BSD_MODULE_CONFIG)" in /*) ;; *) \
		echo "[config] BSD_DRIVER_MODULE_CONFIG must be absolute"; \
		exit 1;; esac
	@mkdir -p "$(ARM64_LOGIN_OVERRIDE)/etc" \
		"$(ARM64_BSD_MODULE_OVERLAY_DIR)" \
		"$(dir $(ARM64_BSD_MODULE_CONFIG_OVERLAY))"
	@awk -F: 'BEGIN { OFS = FS } $$1 == "root" { $$2 = "" } { print }' rootfs/alpine/arm64/etc/shadow > $(ARM64_LOGIN_OVERRIDE)/etc/shadow
	@cp rootfs/alpine/arm64/etc/inittab $(ARM64_LOGIN_OVERRIDE)/etc/inittab
	@printf 'console\nttyAMA0\ntty1\ntty2\ntty3\ntty4\n' > $(ARM64_LOGIN_OVERRIDE)/etc/securetty
	@: > "$(ARM64_BSD_MODULE_CONFIG_OVERLAY)"
	@for module in $(BSD_BRIDGE_LOADABLE_ARM64_MODULES); do \
		name=$$(basename "$$module"); \
		cp "$$module" "$(ARM64_BSD_MODULE_OVERLAY_DIR)/$$name"; \
		printf '%s\n' "$$name" >> \
			"$(ARM64_BSD_MODULE_CONFIG_OVERLAY)"; \
	done
	@touch $@

$(ARM64_ROOTFS): $(ROOTFS_TOOL) $(ARM64_LOGIN_OVERRIDE)/.stamp FORCE
	@mkdir -p $(OUT)
	@echo "[rootfs] creating alpine arm64 ext4 rootfs image from rootfs/alpine/arm64"
	python3 $(ROOTFS_TOOL) --output $@ --size-mb $(ARM64_ROOTFS_SIZE_MB) --fs ext4 --populate-dir rootfs/alpine/arm64 --override-dir $(ARM64_LOGIN_OVERRIDE)

$(ARM64_INITRAMFS): $(INITRAMFS_TOOL) $(INITRAMFS_BASE_MANIFEST) $(ARM64_LOGIN_OVERRIDE)/.stamp FORCE
	@mkdir -p $(ARM64_OUT)
	python3 $(INITRAMFS_TOOL) \
		--source rootfs/alpine/arm64 \
		--overlay $(ARM64_LOGIN_OVERRIDE) \
		--manifest $(INITRAMFS_BASE_MANIFEST) \
		--output $@ \
		--owner 0:0 \
		--mtime 0 \
...
$(ARM64_ESP): $(ARM64_UEFI_EFI) $(ARM64_ROOTFS)
	@mkdir -p $(ARM64_OUT)
	@rm -f $@
	@dd if=/dev/zero of=$@ bs=1048576 count=0 seek=$(ARM64_ESP_SIZE_MB) >/dev/null 2>&1
	mformat -i $@ -F -v EDGEOSARM ::
	mmd -i $@ ::/EFI ::/EFI/BOOT ::/boot
	mcopy -i $@ $(ARM64_UEFI_EFI) ::/EFI/BOOT/BOOTAA64.EFI
	mcopy -i $@ $(CONFIG)/startup-arm64.nsh ::/startup.nsh
	mcopy -i $@ $(ARM64_ROOTFS) ::/boot/rootfs.img
...
arm64-uefi: $(ARM64_ESP)
	@echo "[arm64] built $(ARM64_UEFI_EFI)"
	@echo "[arm64] built $(ARM64_ESP)"
...
$(ARM64_RPI5_ESP): $(ARM64_UEFI_EFI) $(ARM64_INITRAMFS) config/startup-arm64-initramfs.nsh config/cmdline-arm64-initramfs config/config-rpi5.txt
	@mkdir -p $(ARM64_OUT)
	@rm -f $@
	...
	mcopy -i $@ config/startup-arm64-initramfs.nsh ::/startup.nsh
	mcopy -i $@ config/cmdline-arm64-initramfs ::/boot/cmdline
	mcopy -i $@ config/config-rpi5.txt ::/config.txt
	mcopy -i $@ $(ARM64_INITRAMFS) ::/boot/initramfs.img
...
$(ARM64_LINUX_APK):
	@mkdir -p $(ARM64_LINUX_WORK)
	curl -L --fail -o $@ $(ARM64_LINUX_APK_URL)

$(ARM64_LINUX_TREE)/.stamp: $(ARM64_LINUX_APK)
	@rm -rf $(ARM64_LINUX_TREE)
	@mkdir -p $(ARM64_LINUX_TREE)
	tar -xf $< -C $(ARM64_LINUX_TREE)
	@touch $@

$(ARM64_LINUX_INITRAMFS): tools/arm64/mk_linux_initramfs.py $(ARM64_LINUX_TREE)/.stamp
	@mkdir -p $(ARM64_LINUX_OUT)
	@MODDIR=$$(find $(ARM64_LINUX_TREE)/lib/modules -mindepth 1 -maxdepth 1 -type d | head -1); \
	if [ -z "$$MODDIR" ]; then echo "[arm64] missing linux-lts modules"; exit 1; fi; \
	python3 tools/arm64/mk_linux_initramfs.py \
		--rootfs-dir rootfs/alpine/arm64 \
		--modules-dir "$$MODDIR" \
		--root-device /dev/vda \
		--output $@

$(ARM64_LINUX_STARTUP): $(ARM64_LINUX_INITRAMFS)
	@mkdir -p $(ARM64_LINUX_OUT)
	@printf 'fs0:\r\n\\boot\\vmlinuz-lts initrd=\\boot\\initramfs.cpio.gz root=/dev/vda rw rootfstype=ext4 console=ttyAMA0,115200 console=tty0 loglevel=7 ignore_loglevel fbcon=map:0\r\n' > $@

$(ARM64_LINUX_ESP): $(ARM64_LINUX_TREE)/.stamp $(ARM64_LINUX_INITRAMFS) $(ARM64_LINUX_STARTUP)
	@mkdir -p $(ARM64_LINUX_OUT)
	@rm -f $@
	@dd if=/dev/zero of=$@ bs=1m count=128 >/dev/null 2>&1
	mformat -i $@ -F -v EDGELNX64 ::
	mmd -i $@ ::/boot
	mcopy -i $@ $(ARM64_LINUX_KERNEL) ::/boot/vmlinuz-lts
	mcopy -i $@ $(ARM64_LINUX_INITRAMFS) ::/boot/initramfs.cpio.gz
	mcopy -i $@ $(ARM64_LINUX_STARTUP) ::/startup.nsh

arm64-linux-login: $(ARM64_LINUX_ESP) $(ARM64_ROOTFS)
	@echo "[arm64] built real Alpine login boot image $(ARM64_LINUX_ESP)"
	@echo "[arm64] built real Alpine rootfs $(ARM64_ROOTFS)"
...
rootfs-install-userland: $(ROOTFS) $(CMD_BINS)
	@echo "[rootfs] using alpine rootfs profile from $(ROOTFS_ALPINE_DIR)"
	@if [ -d "$(X11_ROOTFS_OVERRIDE)" ]; then \
		echo "[rootfs] applying X11 override from $(X11_ROOTFS_OVERRIDE)"; \
		python3 $(ROOTFS_TOOL) --output $(ROOTFS) --override-dir $(X11_ROOTFS_OVERRIDE) --apply-override-only; \
	else \
		echo "[rootfs] note: X11 override not present (build with make x11-musl)"; \
	fi
	@echo "[rootfs] installing native console userland into alpine image..."
	...
	@if [ -f "$(US_BUILD)/esh" ]; then \
		debugfs -w -R "rm /bin/esh" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "unlink /bin/esh" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "write $(US_BUILD)/esh /bin/esh" $(ROOTFS) >/dev/null || exit 1; \
	fi
	@if [ -f "$(US_BUILD)/getty" ]; then \
		...
		debugfs -w -R "write $(US_BUILD)/getty /sbin/getty" $(ROOTFS) >/dev/null || exit 1; \
	fi
	@echo "[rootfs] preserving Alpine musl loader/libc from $(ROOTFS_ALPINE_DIR)"
...
rootfs-install-userland: $(ROOTFS) $(CMD_BINS)
	...
	@for c in $(CMD_NAMES); do \
		...
		debugfs -w -R "write $(US_BUILD)/$$c /bin/$$c" $(ROOTFS) >/dev/null || exit 1; \
	done
	...
	if [ -n "$$LD_MUSL_PATH" ] && [ "$$LD_MUSL_PATH" != "ld-musl-x86_64.so.1" ] && [ -f "$$LD_MUSL_PATH" ]; then \
		...
		debugfs -w -R "write $$LD_MUSL_PATH /lib/ld-musl-x86_64.so.1" $(ROOTFS) >/dev/null || exit 1; \
	else \
	...
	if [ -n "$$LIBC_SO_PATH" ] && [ "$$LIBC_SO_PATH" != "libc.so" ] && [ -f "$$LIBC_SO_PATH" ]; then \
		...
		debugfs -w -R "write $$LIBC_SO_PATH /lib/libc.so" $(ROOTFS) >/dev/null || exit 1; \
	else \
	...
	@if [ -f "$(MUSL_BUSYBOX_BIN)" ]; then \
		...
		debugfs -w -R "write $(MUSL_BUSYBOX_BIN) /bin/busybox" $(ROOTFS) >/dev/null || exit 1; \
		...
		debugfs -w -R "symlink /bin/sh busybox" $(ROOTFS) >/dev/null || exit 1; \
		...
		for a in $$APPLET_LIST; do \
			...
			debugfs -w -R "symlink /bin/$$a busybox" $(ROOTFS) >/dev/null || exit 1; \
		done; \
	elif [ -f "$(US_BUILD)/esh" ]; then \
		debugfs -w -R "rm /bin/sh" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "unlink /bin/sh" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "write $(US_BUILD)/esh /bin/sh" $(ROOTFS) >/dev/null || exit 1; \
	fi
	@if [ -f "$(DROPBEAR_BIN)" ]; then \
		...
		debugfs -w -R "write $(DROPBEAR_BIN) /bin/dropbear" $(ROOTFS) >/dev/null || exit 1; \
	else \
	...
	@if [ -f "$(DROPBEARKEY_BIN)" ]; then \
		...
		debugfs -w -R "write $(DROPBEARKEY_BIN) /bin/dropbearkey" $(ROOTFS) >/dev/null || exit 1; \
	else \
	...
	@if [ -f "$(TLS_CA_BUNDLE_SRC)" ]; then \
		...
		debugfs -w -R "write $(TLS_CA_BUNDLE_SRC) /etc/ssl/certs/ca-certificates.crt" $(ROOTFS) >/dev/null || exit 1; \
		debugfs -w -R "rm /etc/ssl/cert.pem" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "unlink /etc/ssl/cert.pem" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "symlink /etc/ssl/cert.pem certs/ca-certificates.crt" $(ROOTFS) >/dev/null || exit 1; \
	else \
	...
	@if [ -f "$(OPENSSL_HELPER_BIN)" ]; then \
		...
		debugfs -w -R "write $(OPENSSL_HELPER_BIN) /bin/openssl" $(ROOTFS) >/dev/null || exit 1; \
	else \
	...
	@if [ -f "$(PYTHON3_BIN)" ]; then \
		...
		debugfs -w -R "write $(PYTHON3_BIN) /bin/python3" $(ROOTFS) >/dev/null || exit 1; \
	else \
	...
	@printf 'NAME=EdgeOS\nVERSION=%s\n' "$(KERNEL_RELEASE)" > $(OUT)/os-release
	@debugfs -w -R "rm /etc/os-release" $(ROOTFS) >/dev/null 2>&1 || true
	@debugfs -w -R "unlink /etc/os-release" $(ROOTFS) >/dev/null 2>&1 || true
	@debugfs -w -R "write $(OUT)/os-release /etc/os-release" $(ROOTFS) >/dev/null
	@debugfs -w -R "mkdir /lib64" $(ROOTFS) >/dev/null 2>&1 || true
	@debugfs -w -R "mkdir /lib/x86_64-linux-gnu" $(ROOTFS) >/dev/null 2>&1 || true
	@if [ -f "$(GLIBC_LOADER_SRC)" ]; then \
		echo "[rootfs] installing glibc loader from $(GLIBC_LOADER_SRC)"; \
		debugfs -w -R "rm /lib64/ld-linux-x86-64.so.2" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "unlink /lib64/ld-linux-x86-64.so.2" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "write $(GLIBC_LOADER_SRC) /lib64/ld-linux-x86-64.so.2" $(ROOTFS) >/dev/null || exit 1; \
	else \
		echo "[rootfs] warning: glibc loader not found on host"; \
	fi
	@if [ -f "$(GLIBC_LIBC_SRC)" ]; then \
		echo "[rootfs] installing glibc libc.so.6 from $(GLIBC_LIBC_SRC)"; \
		debugfs -w -R "rm /lib/x86_64-linux-gnu/libc.so.6" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "unlink /lib/x86_64-linux-gnu/libc.so.6" $(ROOTFS) >/dev/null 2>&1 || true; \
		debugfs -w -R "write $(GLIBC_LIBC_SRC) /lib/x86_64-linux-gnu/libc.so.6" $(ROOTFS) >/dev/null || exit 1; \
	else \
		echo "[rootfs] warning: glibc libc.so.6 not found on host"; \
	fi
	@LD_MUSL_PATH="$(MUSL_RUNTIME_LIBDIR)/ld-musl-x86_64.so.1"; \
	LIBC_SO_PATH="$(MUSL_RUNTIME_LIBDIR)/libc.so"; \
	if [ ! -f "$$LD_MUSL_PATH" ] && [ -f "$$LIBC_SO_PATH" ]; then LD_MUSL_PATH="$$LIBC_SO_PATH"; fi; \
...
run: all
	...
	@echo "[run] rootfs profile: $(ROOTFS_PROFILE)"
	@NET_MODE=$${EDGEOS_NET:-tap}; \
	TAP_IF=$${EDGEOS_TAP_IF:-tap0}; \
	USB_MODE=$${EDGEOS_USB:-xhci-mouse}; \
	REBOOT_FLAG=$${EDGEOS_QEMU_REBOOT_FLAG:-}; \
	QEMU_MEM=$${EDGEOS_QEMU_MEM:-$(QEMU_MEM_DEFAULT)}; \
	NETDEV=""; \
	NETNIC=""; \
	if [ "$$NET_MODE" = "tap" ]; then \
		if command -v ip >/dev/null 2>&1 && ip link show dev "$$TAP_IF" >/dev/null 2>&1; then \
			if ip -o link show dev "$$TAP_IF" | grep -q "UP"; then \
				...
				NETNIC="-device e1000,netdev=net0"; \
			else \
				echo "[run] TAP $$TAP_IF exists but is down, falling back to QEMU user networking"; \
				NETDEV="-netdev user,id=net0"; \
				NETNIC="-device e1000,netdev=net0"; \
			fi; \
		else \
			echo "[run] TAP $$TAP_IF not found, falling back to QEMU user networking"; \
			NETDEV="-netdev user,id=net0"; \
			NETNIC="-device e1000,netdev=net0"; \
		fi; \
	else \
	...
	if [ "$$USB_MODE" = "off" ]; then \
		...
		USBDEV=""; \
	elif [ "$$USB_MODE" = "uhci-mouse" ]; then \
		echo "[run] USB: piix3 UHCI + usb-mouse"; \
		USBDEV="-device piix3-usb-uhci,id=usb0 -device usb-mouse,bus=usb0.0"; \
	else \
		echo "[run] USB: qemu-xhci + usb-mouse"; \
		USBDEV="-device qemu-xhci,id=usb0 -device usb-mouse,bus=usb0.0"; \
	fi; \
	qemu-system-x86_64 \
	-enable-kvm \
	...
	-serial stdio \
	-drive file=$(ROOTFS),format=raw,if=none,id=rootfsdisk,cache=none,aio=native,discard=unmap \
	-device nvme,serial=edgeos-rootfs,drive=rootfsdisk \
	-cdrom $(ISO) \
	$$NETDEV \
	$$NETNIC \
	$$USBDEV \
...
run-edgeos:
	@$(MAKE) ROOTFS_PROFILE=edgeos run

run-alpine:
	@$(MAKE) ROOTFS_PROFILE=alpine run

run-arm64-uefi: arm64-uefi
	...
	$(QEMU_AARCH64) \
		...
		-display none \
		-device ramfb \
		-drive if=none,file=$(ARM64_ESP),format=raw,id=esp \
...
run-arm64-uefi-window: arm64-uefi
	@FW=$${EDGEOS_AARCH64_EFI:-/opt/homebrew/share/qemu/edk2-aarch64-code.fd}; \
	if [ ! -f "$$FW" ]; then FW="/opt/homebrew/Cellar/qemu/11.0.2/share/qemu/edk2-aarch64-code.fd"; fi; \
	if [ ! -f "$$FW" ]; then echo "[arm64] missing EDK2 AArch64 firmware; set EDGEOS_AARCH64_EFI=/path/to/edk2-aarch64-code.fd"; exit 1; fi; \
	ACCEL=$${EDGEOS_QEMU_ACCEL:-hvf}; \
	...
	$(QEMU_AARCH64) \
		...
		-m $${EDGEOS_QEMU_MEM:-2048M} \
		-bios "$$FW" \
		-serial file:$(OUT)/arm64/qemu-window-serial.log \
		-display cocoa \
		-monitor none \
		-device ramfb \
		-drive if=none,file=$(ARM64_ESP),format=raw,id=esp \
...
run-arm64-linux-login: arm64-linux-login
	@FW=$${EDGEOS_AARCH64_EFI:-/opt/homebrew/share/qemu/edk2-aarch64-code.fd}; \
	if [ ! -f "$$FW" ]; then FW="/opt/homebrew/Cellar/qemu/11.0.2/share/qemu/edk2-aarch64-code.fd"; fi; \
	if [ ! -f "$$FW" ]; then echo "[arm64] missing EDK2 AArch64 firmware; set EDGEOS_AARCH64_EFI=/path/to/edk2-aarch64-code.fd"; exit 1; fi; \
	$(QEMU_AARCH64) \
		...
		-drive if=none,file=$(ARM64_LINUX_ESP),format=raw,id=esp \
		-device virtio-blk-device,drive=esp \
		-drive if=none,file=$(ARM64_ROOTFS),format=raw,id=rootfsdisk \
		-device virtio-blk-device,drive=rootfsdisk \
		-no-reboot

run-arm64-linux-login-window: arm64-linux-login
	@FW=$${EDGEOS_AARCH64_EFI:-/opt/homebrew/share/qemu/edk2-aarch64-code.fd}; \
	if [ ! -f "$$FW" ]; then FW="/opt/homebrew/Cellar/qemu/11.0.2/share/qemu/edk2-aarch64-code.fd"; fi; \
	if [ ! -f "$$FW" ]; then echo "[arm64] missing EDK2 AArch64 firmware; set EDGEOS_AARCH64_EFI=/path/to/edk2-aarch64-code.fd"; exit 1; fi; \
	$(QEMU_AARCH64) \
		...
		-display cocoa \
		-monitor none \
		-serial file:$(ARM64_LINUX_OUT)/qemu-window-serial.log \
		-device virtio-gpu-pci \
		-drive if=none,file=$(ARM64_LINUX_ESP),format=raw,id=esp \
		-device virtio-blk-device,drive=esp \
		-drive if=none,file=$(ARM64_ROOTFS),format=raw,id=rootfsdisk \
...
rootfs-edgeos:
	@$(MAKE) ROOTFS_PROFILE=edgeos $(ROOTFS)

rootfs-alpine:
	@$(MAKE) ROOTFS_PROFILE=alpine $(OUT)/rootfs-alpine.img

rootfs-alpine-arm64:
	@$(MAKE) $(ARM64_ROOTFS)

rootfs-debian:
	@python3 $(DEBIAN_ROOTFS_TOOL) \
		--architecture $(ROOTFS_ARCH) \
		--suite $(DEBIAN_SUITE) \
		--size-mb $(DEBIAN_ROOTFS_SIZE_MB) \
		--output $(DEBIAN_ROOTFS_IMAGE) \
		--rootfs-dir $(DEBIAN_ROOTFS_DIR)

.PHONY: x86_64-debian-live-rootfs x86_64-debian-live-initramfs x86_64-debian-live-iso

$(DEBIAN_LIVE_SQUASHFS): $(DEBIAN_ROOTFS_TOOL) FORCE
	@python3 $(DEBIAN_ROOTFS_TOOL) \
		--architecture x86_64 \
		--profile workstation \
		--suite $(DEBIAN_SUITE) \
		...
		--output $@ \
		--rootfs-dir $(DEBIAN_LIVE_ROOTFS_DIR)

x86_64-debian-live-rootfs: $(DEBIAN_LIVE_SQUASHFS)
	@echo "[x86_64] built $(DEBIAN_LIVE_SQUASHFS)"

$(DEBIAN_LIVE_INIT_LAUNCHER): $(DEBIAN_LIVE_INIT_LAUNCHER_SOURCE)
	@mkdir -p $(dir $@)
	@$(CC) -m64 -nostdlib -static -no-pie -fno-pie \
		-fno-stack-protector -fno-builtin -Wl,--build-id=none \
		-Wl,-z,noexecstack -Wl,-e,_start $< -o $@

$(DEBIAN_LIVE_INITRAMFS): $(DEBIAN_LIVE_INITRAMFS_TOOL) \
		$(DEBIAN_LIVE_INIT_LAUNCHER) \
		$(INITRAMFS_TOOL) $(DEBIAN_LIVE_BUSYBOX) $(DEBIAN_LIVE_MUSL_LOADER)
	@python3 $(DEBIAN_LIVE_INITRAMFS_TOOL) \
		--busybox $(DEBIAN_LIVE_BUSYBOX) \
		--loader $(DEBIAN_LIVE_MUSL_LOADER) \
		--launcher $(DEBIAN_LIVE_INIT_LAUNCHER) \
		--output $@

x86_64-debian-live-initramfs: $(DEBIAN_LIVE_INITRAMFS)
	@echo "[x86_64] built $(DEBIAN_LIVE_INITRAMFS)"

$(DEBIAN_LIVE_ISO): $(TARGET) $(DEBIAN_LIVE_SQUASHFS) \
		$(DEBIAN_LIVE_INITRAMFS) \
		$(DEBIAN_LIVE_ISO_TOOL) $(DEBIAN_LIVE_GRUB_CONFIG)
	@python3 $(DEBIAN_LIVE_ISO_TOOL) \
		--kernel $(TARGET) \
		--initramfs $(DEBIAN_LIVE_INITRAMFS) \
		--squashfs $(DEBIAN_LIVE_SQUASHFS) \
		--grub-config $(DEBIAN_LIVE_GRUB_CONFIG) \
		--output $@

x86_64-debian-live-iso: $(DEBIAN_LIVE_ISO)
	@echo "[x86_64] built $(DEBIAN_LIVE_ISO)"
...
x11-musl: musl
	@echo "[ports] building X11/Xorg with musl"
	@XSERVER_DIR="$(abspath $(XSERVER_DIR))" \
	LIBXCB_DIR="$(abspath $(LIBXCB_DIR))" \
	X11_PREFIX="$(abspath $(X11_PREFIX))" \
	X11_SYSROOT="$(abspath $(X11_SYSROOT))" \
	X11_ROOTFS_OVERRIDE="$(abspath $(X11_ROOTFS_OVERRIDE))" \
	X11_COMPONENTS="$(X11_COMPONENTS)" \
	bash $(X11_BUILD_SCRIPT)
