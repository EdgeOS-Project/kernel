...
int edge_linux_syscall_map(edge_linux_syscall_architecture_t architecture,
    ...
    if (architecture == EDGE_LINUX_ARCH_X32) {
        return edge_linux_syscall_lookup(
            edge_linux_x32_numbers,
            sizeof(edge_linux_x32_numbers) /
...
typedef struct edge_linux_bpf_program_bind_map_attribute {
    uint32_t program_descriptor;
    uint32_t map_descriptor;
    uint32_t flags;
...
static int64_t edge_linux_bpf_map_create(
    ...
    if (attribute.map_type == KERNEL_BPF_MAP_TYPE_ARRAY_OF_MAPS ||
        ...
        request.inner_map_object_id = kernel_bpf_descriptor_object(
            (int32_t)attribute.inner_map_descriptor,
            KERNEL_BPF_OBJECT_MAP);
        if (request.inner_map_object_id < 0)
            return request.inner_map_object_id;
    }
...
static int edge_linux_bpf_sk_storage_element(
    ...
    if (status < 0) return status;
    status = (int)kernel_fd_operation_socket(&lease, &request, &result);
    if (status == 0)
        status = kernel_fd_operation_description_id(&lease, &identity);
    if (status == 0) {
        if (command == EDGE_LINUX_BPF_MAP_LOOKUP_ELEM)
            status = kernel_bpf_sk_storage_lookup(
                object_id, identity, value, flags);
        else if (command == EDGE_LINUX_BPF_MAP_UPDATE_ELEM)
            status = kernel_bpf_sk_storage_update(
                object_id, identity, value, flags);
        else
            status = kernel_bpf_sk_storage_delete(
                object_id, identity);
    }
    release_status = kernel_fd_operation_release(&lease);
    if (status == 0 && release_status < 0) status = release_status;
    return status;
}
...
static int edge_linux_bpf_task_storage_element(
        uint32_t command, int object_id, const void *key,
        void *value, uint64_t flags) {
    ...
    if (!key) return -EDGE_LINUX_EFAULT;
    memcpy(&descriptor, key, sizeof(descriptor));
    status = kernel_pidfd_target(descriptor, &tid, &pidfd_flags);
    if (status < 0) return status;
    if (kernel_proc_task_view_get(tid, &task) < 0)
        return -EDGE_LINUX_ENOENT;
    if (command == EDGE_LINUX_BPF_MAP_LOOKUP_ELEM)
        return kernel_bpf_task_storage_lookup(
            object_id, task.tid, task.start_time_ticks, value, flags);
    if (command == EDGE_LINUX_BPF_MAP_UPDATE_ELEM)
        return kernel_bpf_task_storage_update(
            object_id, task.tid, task.start_time_ticks, value, flags);
    return kernel_bpf_task_storage_delete(
        object_id, task.tid, task.start_time_ticks);
}
...
static int64_t edge_linux_bpf_map_element(
    ...
    uint8_t *key = 0;
    uint8_t *value = 0;
    uint32_t key_pages = 0u;
    uint32_t value_size = 0u;
    uint32_t value_pages = 0u;
    int object_id;
    int status;
    ...
    if (!attribute.value && command != EDGE_LINUX_BPF_MAP_DELETE_ELEM)
        return -EDGE_LINUX_EFAULT;
    if (command == EDGE_LINUX_BPF_MAP_GET_NEXT_KEY) {
        value_size = info.key_size;
    } else if (command != EDGE_LINUX_BPF_MAP_DELETE_ELEM) {
        status = kernel_bpf_map_value_buffer_size(
            object_id, attribute.flags, &value_size);
        if (status < 0) return status;
    }
    if (info.key_size) {
        key_pages = (info.key_size + 4095u) / 4096u;
        key = (uint8_t *)arch_vm_alloc_pages(key_pages);
        if (!key) return -EDGE_LINUX_ENOMEM;
    }
    if (command != EDGE_LINUX_BPF_MAP_DELETE_ELEM) {
        value_pages = (value_size + 4095u) / 4096u;
        value = (uint8_t *)arch_vm_alloc_pages(value_pages);
        if (!value) {
            edge_linux_bpf_free_pages(key, key_pages);
            return -EDGE_LINUX_ENOMEM;
        }
    }
    ...
    if (command == EDGE_LINUX_BPF_MAP_LOOKUP_ELEM) {
        ...
        if (info.type == KERNEL_BPF_MAP_TYPE_CGRP_STORAGE) {
            ...
            status = edge_linux_bpf_inode_storage_element(
                command, object_id, key, value, attribute.flags);
        } else if (info.type == KERNEL_BPF_MAP_TYPE_TASK_STORAGE) {
            status = edge_linux_bpf_task_storage_element(
                command, object_id, key, value, attribute.flags);
        } else {
...
out:
    edge_linux_bpf_free_pages(value, value_pages);
    edge_linux_bpf_free_pages(key, key_pages);
    return status;
}
    ...
    while (processed < requested) {
        ...
        if (command == EDGE_LINUX_BPF_MAP_LOOKUP_BATCH ||
            ...
            if (edge_linux_copy_to_user(
                ...
                edge_linux_copy_to_user(
                    context, value_address, value, value_size) < 0) {
                status = -EDGE_LINUX_EFAULT;
                break;
            }
            ++processed;
...
static int64_t edge_linux_bpf_object_info(
    ...
    if (kind == KERNEL_BPF_OBJECT_MAP) {
        ...
        uint32_t map_ids[EDGE_RUNTIME_MAX_BPF_OBJECTS];
        uint32_t actual_map_count = 0u;
        ...
        if (status < 0) return status;
        status = kernel_bpf_program_map_ids(
            object_id, map_ids, EDGE_RUNTIME_MAX_BPF_OBJECTS,
            &actual_map_count);
        if (status < 0) return status;
        if (requested.map_id_count && actual_map_count) {
            uint32_t map_count = requested.map_id_count;

            if (map_count > actual_map_count)
                map_count = actual_map_count;
            if (edge_linux_copy_to_user(
                    context, requested.map_ids, map_ids,
                    (uint64_t)map_count * sizeof(map_ids[0])) < 0)
                return -EDGE_LINUX_EFAULT;
        }
        ...
        linux_info.map_id_count = actual_map_count;
        linux_info.map_ids = requested.map_ids;
...
static int64_t edge_linux_bpf_program_attachment(
    ...
    int relative_object_id = -1;
    int relative_retained = 0;
    int status;
    ...
    if (command == EDGE_LINUX_BPF_PROG_ATTACH) {
        if (attribute.attach_flags &
            ~(KERNEL_BPF_F_ALLOW_OVERRIDE |
              ...
              KERNEL_BPF_F_LINK))
            return -EDGE_LINUX_EINVAL;
        if (attribute.attach_flags & KERNEL_BPF_F_LINK)
            return -EDGE_LINUX_EINVAL;
        ...
        if (attribute.relative_descriptor ||
            (attribute.attach_flags & KERNEL_BPF_F_ID)) {
            if (attribute.attach_flags & KERNEL_BPF_F_ID) {
                relative_object_id = kernel_bpf_object_from_user_id(
                    KERNEL_BPF_OBJECT_PROGRAM,
                    attribute.relative_descriptor);
                relative_retained = relative_object_id >= 0;
            } else if (attribute.relative_descriptor > INT32_MAX) {
                return -EDGE_LINUX_EBADF;
            } else {
                relative_object_id = kernel_bpf_descriptor_object(
                    (int32_t)attribute.relative_descriptor,
                    KERNEL_BPF_OBJECT_PROGRAM);
            }
            if (relative_object_id < 0)
                return relative_object_id;
        }
        status = edge_linux_bpf_target_description(
            attribute.target_descriptor, &target);
        if (status < 0) goto attachment_out;
        if (!target.superblock || !target.inode) {
            status = -EDGE_LINUX_EBADF;
            goto attachment_out;
        }
        status = cgroupfs_bpf_program_attach(
            target.superblock, target.inode, object_id,
            attribute.attach_flags & ~KERNEL_BPF_F_ID,
            replace_object_id, relative_object_id,
            attribute.expected_revision);
attachment_out:
        if (relative_retained)
            kernel_bpf_object_release(relative_object_id);
        return status;
    }
...
static int64_t edge_linux_bpf_link_command(
    ...
    if (command == EDGE_LINUX_BPF_LINK_CREATE) {
        ...
        if (attribute.attach_type != KERNEL_BPF_CGROUP_DEVICE ||
            attribute.padding ||
            (attribute.flags & ~(KERNEL_BPF_F_BEFORE |
                                 KERNEL_BPF_F_AFTER |
                                 KERNEL_BPF_F_ID |
                                 KERNEL_BPF_F_PREORDER |
                                 KERNEL_BPF_F_LINK)))
            return -EDGE_LINUX_EINVAL;
        if ((attribute.relative_descriptor &&
             !(attribute.flags & KERNEL_BPF_F_LINK)) ||
            ((attribute.flags & KERNEL_BPF_F_ID) &&
             !(attribute.flags & KERNEL_BPF_F_LINK)) ||
            ((attribute.flags & KERNEL_BPF_F_LINK) &&
             !attribute.relative_descriptor &&
             !(attribute.flags & KERNEL_BPF_F_ID)))
        ...
        if (attribute.relative_descriptor ||
            (attribute.flags & (KERNEL_BPF_F_ID |
                                KERNEL_BPF_F_LINK))) {
            if (attribute.flags & KERNEL_BPF_F_ID) {
                relative_object_id = kernel_bpf_object_from_user_id(
                    KERNEL_BPF_OBJECT_LINK,
                    attribute.relative_descriptor);
                relative_retained = relative_object_id >= 0;
        ...
        status = edge_linux_bpf_target_description(
            attribute.target_descriptor, &target);
        if (status < 0) goto link_create_out;
        if (!target.superblock || !target.inode) {
            status = -EDGE_LINUX_EBADF;
            goto link_create_out;
        }
        link_object_id = cgroupfs_bpf_link_create(
            target.superblock, target.inode, program_object_id,
            attribute.attach_type,
            attribute.flags & ~KERNEL_BPF_F_ID,
            relative_object_id, attribute.expected_revision);
        if (link_object_id < 0) {
            status = link_object_id;
            goto link_create_out;
        }
        status = kernel_bpf_create_descriptor(link_object_id);
link_create_out:
...
static int64_t edge_linux_bpf_program_bind_map(
        edge_linux_syscall_context_t *context, uint64_t user_attribute,
        uint32_t attribute_size) {
    edge_linux_bpf_program_bind_map_attribute_t attribute;
    int program_object_id;
    int map_object_id;
    int status;
    ...
    return kernel_bpf_program_bind_map(
...
static int64_t edge_linux_bpf_program_query(
    ...
    for (uint32_t index = 0u; index < copied; ++index) {
        ...
        if (edge_linux_copy_to_user(
            ...
            return -EDGE_LINUX_EFAULT;
        if (attribute.program_attach_flags) {
            uint32_t program_attach_flags = attach_flags[index] &
                (KERNEL_BPF_F_ALLOW_OVERRIDE |
                 KERNEL_BPF_F_ALLOW_MULTI);
            ...
            if (edge_linux_copy_to_user(
                ...
                &program_attach_flags,
                sizeof(program_attach_flags)) < 0)
                return -EDGE_LINUX_EFAULT;
        }
    }
    attribute.program_count = count;
    attribute.attach_flags = count ?
        attach_flags[0] & (KERNEL_BPF_F_ALLOW_OVERRIDE |
                           KERNEL_BPF_F_ALLOW_MULTI) : 0u;
    if (edge_linux_copy_to_user(
...
#define EDGE_LINUX_IORING_OP_READ_FIXED 4u
#define EDGE_LINUX_IORING_OP_WRITE_FIXED 5u
...
#define EDGE_LINUX_IORING_OP_PROVIDE_BUFFERS 31u
#define EDGE_LINUX_IORING_OP_REMOVE_BUFFERS 32u
...
#define EDGE_LINUX_IORING_REGISTER_BUFFERS      0u
#define EDGE_LINUX_IORING_UNREGISTER_BUFFERS    1u
...
#define EDGE_LINUX_IORING_REGISTER_BUFFERS2      15u
...
static int32_t edge_linux_io_uring_execute_descriptor(
    ...
    case EDGE_LINUX_IORING_OP_WRITE_FIXED:
        if (submission->splice_descriptor)
            result = -EDGE_LINUX_EINVAL;
        else if (submission->length > EDGE_LINUX_MAX_RW_COUNT)
            result = -EDGE_LINUX_EFAULT;
        else {
            result = kernel_io_uring_fixed_buffer_validate(
                ring_id, submission->buffer_index,
...
static int32_t edge_linux_io_uring_manage_buffers(
        edge_linux_syscall_context_t *context,
        int32_t ring_id,
        const struct edge_linux_io_uring_sqe *submission) {
    uint32_t count;
    ...
    if (submission->flags & (EDGE_LINUX_IOSQE_FIXED_FILE |
                             EDGE_LINUX_IOSQE_BUFFER_SELECT))
        return -EDGE_LINUX_EOPNOTSUPP;
    if (submission->ioprio || submission->personality ||
        submission->splice_descriptor || submission->address3 ||
        submission->reserved2 || submission->operation_flags)
        return -EDGE_LINUX_EINVAL;
    if (submission->descriptor <= 0 ||
        (uint32_t)submission->descriptor > UINT16_MAX + 1u)
        return -EDGE_LINUX_EINVAL;
    count = (uint32_t)submission->descriptor;

    if (submission->opcode == EDGE_LINUX_IORING_OP_REMOVE_BUFFERS) {
        if (submission->offset || submission->address ||
            submission->length)
            return -EDGE_LINUX_EINVAL;
        return kernel_io_uring_provided_buffers_remove(
            ring_id, submission->buffer_index, count);
    }
    if (submission->opcode != EDGE_LINUX_IORING_OP_PROVIDE_BUFFERS)
        return -EDGE_LINUX_EINVAL;
    if (!submission->length) return -EDGE_LINUX_EINVAL;
    if (submission->offset > UINT16_MAX)
        return -EDGE_LINUX_E2BIG;
    if (count > UINT16_MAX + 1u - (uint32_t)submission->offset)
        return -EDGE_LINUX_EINVAL;
    if ((uint64_t)count > UINT64_MAX / submission->length)
        return -EDGE_LINUX_EOVERFLOW;
    {
        uint64_t total_length =
            (uint64_t)count * submission->length;

        if (submission->address > UINT64_MAX - total_length)
            return -EDGE_LINUX_EOVERFLOW;
        if (edge_linux_validate_user_range(
                context, submission->address, total_length, 1) < 0)
            return -EDGE_LINUX_EFAULT;
    }
    return kernel_io_uring_provided_buffers_add(
        ring_id, submission->buffer_index,
        (uint16_t)submission->offset, submission->address,
        submission->length, count);
}

static int edge_linux_io_uring_select_buffer(
        edge_linux_syscall_context_t *context,
        int32_t ring_id,
        const struct edge_linux_io_uring_sqe *submission,
    ...
    *provided_ring = 0;
    ...
    if (submission->opcode != EDGE_LINUX_IORING_OP_READ &&
        ...
        submission->opcode != EDGE_LINUX_IORING_OP_RECV)
        return -EDGE_LINUX_EOPNOTSUPP;
    if (!kernel_fd_is_open(submission->descriptor))
        return -EDGE_LINUX_EBADF;
    if (submission->opcode == EDGE_LINUX_IORING_OP_READV) {
        struct edge_linux_iovec vector;
...
static int edge_linux_io_uring_probe_supported(uint8_t opcode) {
    return opcode == EDGE_LINUX_IORING_OP_NOP ||
           ...
           opcode == EDGE_LINUX_IORING_OP_READ_FIXED ||
           opcode == EDGE_LINUX_IORING_OP_WRITE_FIXED ||
           ...
           opcode == EDGE_LINUX_IORING_OP_PROVIDE_BUFFERS ||
           opcode == EDGE_LINUX_IORING_OP_REMOVE_BUFFERS ||
...
static int64_t edge_linux_io_uring_files_update_user(
    ...
    if (kernel_io_current_vector_scratch(&scratch) < 0 ||
        !scratch.vectors)
        return -EDGE_LINUX_ENOMEM;
    tag_offset = ((uint64_t)count * sizeof(int32_t) +
                  sizeof(uint64_t) - 1u) &
                 ~(uint64_t)(sizeof(uint64_t) - 1u);
    required = tag_offset +
        (user_tags ? (uint64_t)count * sizeof(uint64_t) : 0u);
    if (required >
        (uint64_t)scratch.capacity * sizeof(scratch.vectors[0]))
        return -EDGE_LINUX_ENOMEM;
    descriptors = (int32_t *)(void *)scratch.vectors;
    tags = (uint64_t *)(void *)(
        (uint8_t *)(void *)scratch.vectors + tag_offset);
    memset(scratch.vectors, 0, (uint32_t)required);
...
static int edge_linux_io_uring_files_register_user(
        edge_linux_syscall_context_t *context, int32_t ring_id,
        uint64_t user_descriptors, uint64_t user_tags, uint32_t count) {
    kernel_io_vector_scratch_t scratch;
    int32_t *descriptors;
    uint64_t *tags;
    uint64_t tag_offset;
    ...
    for (uint32_t index = 0; index < count; ++index) {
        ...
        if (user_tags) {
            if ((uint64_t)index >
                (UINT64_MAX - user_tags) / sizeof(tags[index]))
                return -EDGE_LINUX_EFAULT;
            source = user_tags +
                     (uint64_t)index * sizeof(tags[index]);
            if (edge_linux_copy_from_user(
        ...
        if (user_descriptors) {
            if ((uint64_t)index >
                (UINT64_MAX - user_descriptors) /
                    sizeof(descriptors[index]))
                return -EDGE_LINUX_EFAULT;
            source = user_descriptors +
                     (uint64_t)index * sizeof(descriptors[index]);
            if (edge_linux_copy_from_user(
        ...
        if (descriptors[index] >= 0 &&
            ...
            return -EDGE_LINUX_EBADF;
        if (descriptors[index] == -1 && user_tags && tags[index])
            return -EDGE_LINUX_EINVAL;
    }
    return user_tags ?
        kernel_io_uring_files_register_tagged(
            ring_id, descriptors, tags, count) :
        kernel_io_uring_files_register(ring_id, descriptors, count);
}

static int edge_linux_io_uring_buffer_access_validate(
        edge_linux_syscall_context_t *context,
    ...
    if (!buffer->iov_base || !buffer->iov_len)
        return 0;
    if (buffer->iov_len > (UINT64_C(1) << 40) ||
        buffer->iov_len > UINT64_MAX - buffer->iov_base)
        return 0;
    if (edge_linux_validate_user_range(
            context, buffer->iov_base, buffer->iov_len, 1) < 0)
        return -EDGE_LINUX_EFAULT;
    last_address = buffer->iov_base + buffer->iov_len - 1u;
    if (edge_linux_copy_from_user(
            context, &first, buffer->iov_base, sizeof(first)) < 0 ||
        edge_linux_copy_to_user(
            context, buffer->iov_base, &first, sizeof(first)) < 0)
        return -EDGE_LINUX_EFAULT;
    if (last_address != buffer->iov_base &&
        (edge_linux_copy_from_user(
             context, &last, last_address, sizeof(last)) < 0 ||
         edge_linux_copy_to_user(
             context, last_address, &last, sizeof(last)) < 0))
        return -EDGE_LINUX_EFAULT;
    return 0;
}
...
static int edge_linux_io_uring_buffers_register_user(
    ...
    kernel_io_vector_scratch_t scratch;
    struct edge_linux_iovec *buffers;
    uint64_t *tags;
    uint64_t required;

    if (!count || count > KERNEL_IO_URING_MAX_FIXED_BUFFERS)
        return -EDGE_LINUX_EINVAL;
    if (kernel_io_current_vector_scratch(&scratch) < 0 ||
        !scratch.vectors)
        return -EDGE_LINUX_ENOMEM;
    required = (uint64_t)count * sizeof(buffers[0]) +
               (user_tags ?
                    (uint64_t)count * sizeof(tags[0]) : 0u);
    if (required >
        (uint64_t)scratch.capacity * sizeof(scratch.vectors[0]))
        return -EDGE_LINUX_ENOMEM;
    buffers = scratch.vectors;
    tags = (uint64_t *)(void *)(buffers + count);
    memset(buffers, 0, (uint32_t)required);
    for (uint32_t index = 0; index < count; ++index) {
        uint64_t source;
        int result;
        ...
        if (user_tags) {
            source = user_tags +
                     (uint64_t)index * sizeof(tags[index]);
            if (source < user_tags || edge_linux_copy_from_user(
                    context, &tags[index], source,
                    sizeof(tags[index])) < 0)
                return -EDGE_LINUX_EFAULT;
        }
...
static int64_t edge_linux_io_uring_buffers_update_user(
        edge_linux_syscall_context_t *context, int32_t ring_id,
        uint32_t offset, uint64_t user_buffers,
        uint64_t user_tags, uint32_t count) {
    uint32_t processed = 0u;
    int result = 0;

    if (!count) return -EDGE_LINUX_EINVAL;
    if (!user_buffers) return -EDGE_LINUX_EFAULT;
    ...
    for (; processed < count; ++processed) {
        ...
        if (user_tags) {
            source = user_tags +
                (uint64_t)processed * sizeof(tag);
            if (source < user_tags || edge_linux_copy_from_user(
                    context, &tag, source, sizeof(tag)) < 0) {
                result = -EDGE_LINUX_EFAULT;
                break;
            }
        }
...
static int64_t edge_linux_io_uring_register_pbuf_ring(
    ...
    if (registration.flags & EDGE_LINUX_IORING_PBUF_RING_MMAP)
        return kernel_io_uring_pbuf_ring_register(
            ring_id, registration.group_id, 0u, 0u,
...
static int64_t edge_linux_sys_io_uring_register(
    ...
    if (opcode == EDGE_LINUX_IORING_REGISTER_BUFFERS) {
        ...
        return edge_linux_io_uring_buffers_register_user(
            context, ring_id, argument, 0, operation_count);
    }
    if (opcode == EDGE_LINUX_IORING_UNREGISTER_BUFFERS) {
        if (argument || operation_count) return -EDGE_LINUX_EINVAL;
        return kernel_io_uring_buffers_unregister(ring_id);
    }
    if (opcode == EDGE_LINUX_IORING_REGISTER_FILES) {
        if (!argument) return -EDGE_LINUX_EFAULT;
        return edge_linux_io_uring_files_register_user(
            context, ring_id, argument, 0, operation_count);
    }
    ...
    if (opcode == EDGE_LINUX_IORING_REGISTER_FILES2) {
        ...
        if (registration.count > KERNEL_IO_URING_MAX_FIXED_FILES)
            return -EDGE_LINUX_EMFILE;
        return edge_linux_io_uring_files_register_user(
            context, ring_id, registration.data,
            registration.tags, registration.count);
    }
    ...
    if (opcode == EDGE_LINUX_IORING_REGISTER_BUFFERS2) {
        struct edge_linux_io_uring_resource_register registration;
    ...
    if (opcode == EDGE_LINUX_IORING_REGISTER_BUFFERS_UPDATE) {
        struct edge_linux_io_uring_resource_update2 update;
        ...
        if (!update.count || update.reserved || update.reserved2)
            return -EDGE_LINUX_EINVAL;
        return edge_linux_io_uring_buffers_update_user(
            context, ring_id, update.offset, update.data,
            update.tags, update.count);
    }
