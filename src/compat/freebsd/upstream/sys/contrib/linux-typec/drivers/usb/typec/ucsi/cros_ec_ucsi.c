...
static int cros_ec_ucsi_probe(struct platform_device *pdev)
{
	(void)pdev;
	return -ENODEV;
}

static struct platform_driver cros_ec_ucsi_driver = {
	.driver = {
		.name = "cros-ec-ucsi",
	},
	.probe = cros_ec_ucsi_probe,
};

module_platform_driver(cros_ec_ucsi_driver);
MODULE_LICENSE("GPL");
