...
#else
/*
 * FreeBSD LinuxKPI aliases struct acpi_device to BSD device_t and does not
 * expose Linux's _PLD checksum. Keep the Type-C core usable without creating
 * ACPI component links that cannot be represented by the BSD device model.
 */
int typec_link_ports(struct typec_port *con)
{
	(void)con;
