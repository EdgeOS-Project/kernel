...
static void syscall_maybe_pump_fbdev_mmap(void) {
     ...
     *
     * Refresh decisions depend only on active fbdev mmap ownership, never on
     * process names or userspace layout.
     */
...
static int syscall_apply_seccomp(task_t *task, REGISTERS *r, uint64_t nr,
    ...
    if (!task || !r || !result_out || !task->seccomp.length ||
        nr == SYS_rt_sigreturn || nr == SYS_x32_rt_sigreturn)
        return 0;
...
void edgeos_x86_64_syscall_dispatch(REGISTERS *r) {
    ...
    if (nr == SYS_x32_rt_sigreturn) {
        r->rax = do_sys_x32_rt_sigreturn(r);
